...
export default function AdminPageComponent() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: concierge } = await supabase
        ...
        .select('*')
        .order('submitted_at', { ascending: false });
      setConciergeRequests(concierge || []);

      const combined = [
        ...(apps || []).map(r => ({ source: 'Application', ...r })),
        ...(concierge || []).map(r => ({ source: 'Concierge', ...r }))
      ];
  ...
  const filteredCombined = filterRows(combinedLeads, combinedSearch);
  const filteredApplications = filterRows(submissions, applicationsSearch);
  const filteredConcierge = filterRows(conciergeRequests, conciergeSearch);
  ...
  return (
    <div>
      ...
      <div style={tabNavStyle}>
        <button style={activeTab === 'combined' ? tabActiveStyle : tabStyle} onClick={() => handleTabChange('combined')}>
          All Leads (Combined)
        </button>
      ...
      {activeTab === 'combined' && (
        <Section title="All Leads (Combined)">
          <SearchBox value={combinedSearch} onChange={setCombinedSearch} />
          <DownloadButton label="Download All Leads CSV"
            onClick={() => downloadCSV(filteredCombined, 'combined_leads.csv',
      ...
      {activeTab === 'applications' && (
        <Section title="Applications">
          ...
          <DownloadButton label="Download Applications CSV"
      ...
      {activeTab === 'concierge' && (
        <Section title="Concierge Requests">
          ...
          <DownloadButton label="Download Concierge Requests CSV"
            onClick={() => downloadCSV(filteredConcierge, 'concierge_requests.csv',
